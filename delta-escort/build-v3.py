#!/usr/bin/env python3
"""Calibrate the existing static reference build; preserve every source poster.
Only anonymous aggregates are rendered. No user photos or account identifiers.
"""
...
assert not list(ROOT.glob('*0e7dfc87*'))
...
<details class="case-review"><summary>档位为什么变了 · 矿工、爬塔、AW单的敏感性</summary><div class="case-body">
<h3>黄金588与988：额外补是真的，也不是无条件选988</h3><p>__MINER__</p>
<h3>永恒之塔：低撤离可能只增加长尾，不增加实际收益</h3><p>__TOWER__</p>
<h3>战争1088：撤销把三AW视为稳定稀缺条件的溢价</h3><p>__WAR__</p>
</div></details>
<details class="case-review"><summary>证据边界与来源</summary><div class="case-body"><p>__METHOD__</p><p>__SCOPE__</p><ul class="case-links">__LINKS__</ul><p class="muted">__PRIVACY__</p></div></details>
</section>
'''
for key,value in {
...
def patch(old,new,expected=1):
    ...
    if expected is not None and n != expected:
        raise RuntimeError('Base build changed: expected '+str(expected)+' occurrence(s), found '+str(n)+' of '+old[:85])
...
# Preserve old deep links: #play-2 continues to identify War even though rank is 25.
...
# Fail closed if the enhancement point is not present. The resulting HTML is
# still fully readable with scripts disabled.
...
with sync_playwright() as pw:
    ...
    for width in [360,390,1440]:
        ...
        assert page.locator('#play-12').get_attribute('data-name') == '小香手'
        assert page.locator('#play-12').get_attribute('data-rank') == '18'
...
print('V3 validation passed: 36 updated ranks, 39 rules sections, 170 tier records, 40 unchanged original posters; no uploaded user images.')
